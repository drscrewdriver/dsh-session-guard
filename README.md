...
- 顶部工具栏三个文字按钮：
  1. `新建畅跑任务`——展开 / 收起内联表单（`开始` / `结束`，各为**双月日期区间日历** + 小时下拉，**小时粒度**：开始小时 H = `H:00`，结束小时 H = `H:59`，即**整个结束小时都算在内**），带 `确定` / `取消`；
...
- 面板打开时用**当前时刻**播种（开始 = 本小时整点 `H:00`，结束 = **下一个小时**的 `H+1:59`——即两个钟点小时，跨天则顺延到下一天）——「新建畅跑任务」最常见的意图就是现在就开始；小时下拉与日历并列保留以提供小时精度，表单里的 `现在` 按钮可随时按当前时刻**重新播种**。
...
- 畅跑任务的起止时刻同样按 `timezone` 解释（小时精度，绝对时刻；开始小时 = `H:00`、结束小时 = `H:59`，即结束小时整点包含，「结束 23 时」= `23:59`）；
...
```bash
npm test   # node --test "tests/*.test.mjs"（397 通过：时区/峰谷策略/配置文件/周末/状态机/会话门/畅跑/桥接/重试）
```
