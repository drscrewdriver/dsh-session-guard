...
- 上部ツールバーの 3 つのテキストボタン：
  1. `新建畅跑任务`——インラインフォーム（`开始` / `结束`、それぞれネイティブの日付ピッカー + 時間セレクト、**時間粒度**）を開閉。`确定` / `取消` 付き；
...
- タスクは**絶対起止時刻**、半開区間 `[from, to)`、**時間精度**。フォームの値は設定された `timezone` で解釈されます（パネルがそのタイムゾーンを表示。`peakWindows` と週末ルールと同じタイムゾーン）；
...
```bash
npm test   # node --test "tests/*.test.mjs"（368 件成功：タイムゾーン/峰谷ポリシー/設定ファイル/週末/状態機械/セッションゲート/畅跑/ブリッジ/リトライ）
```
