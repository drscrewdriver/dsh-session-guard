...
- 상단 툴바의 텍스트 버튼 3개:
  1. `新建畅跑任务` — 인라인 폼(`开始` / `结束`, 각각 **듀얼 먼스 날짜 범위 달력** + 시간 셀렉트, **시간 단위**)을 열고 닫음. `确定` / `取消` 포함;
...
- 패널을 열 때 **현재 시각**으로 초기화합니다(시작 = 현재 시의 정각, 끝 = +2시간. 날을 넘기면 다음 날로 이월) — "畅跑 작업 생성"의 의도는 거의 항상 "지금 시작"이기 때문입니다. 시간 셀렉트는 시간 정밀도를 위해 달력과 나란히 남고, 폼의 `现在` 버튼으로 언제든 현재 시각으로 **다시 시드**할 수 있습니다.
...
- 畅跑 작업의 기시각/종시각도 마찬가지로 `timezone`으로 해석됩니다(시간 정밀도·절대 시각)；
