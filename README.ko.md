...
> dist-tag **`dsh-0.1.7`**. `package.json`과 `dsh.plugin.json`의 `engines.dsh`, 그리고
> 4개의 `@deepseek-ai/dsh-client-*` peer는 모두 `>=0.1.7-rc.1 <0.1.8-0`으로 통일되어 있습니다.
> 과거 README는 "2.x / 3.x"를 **라인 호칭**으로 사용했지만, 이는 서술 관습이며
> **레지스트리에서 받을 수 있는 버전 번호가 아닙니다**. npm 버전 `0.3.1`(0.1.2 라인),
> `3.0.0`/`3.0.1`(0.1.5 라인), `3.1.1`(0.1.7 라인)을 기준으로 하세요.
>
...
>
> **0.1.5 적응(compat/0.1.5 브랜치, npm 3.0.0)**: ① `agent.followup` 이중 경로, ② 재개 메시지의
> `source.form: 'instructions'`, ③ `webServer.register`의 try/catch, ④ `session.events` 배열 접근자 제거에
> 따른 `snapshotEvents()` 주 경로 + 레거시 배열 폴백. 0.1.5의 `WebRoute`(exact/prefix + SSE) 계약은 불변이며
> 클라이언트 `fetch('/session-guard/...')`에 `/api` 접두사가 필요 없음을 확인했습니다. 아래 호환표는
> 0.1.1 / 0.1.2 두 세대에 걸쳐 있지만 **본 라인이 주장하는 것은 0.1.7 행뿐**입니다. 본 라인은 설정면을
> 선언식(`Config` + `.volatile()`)으로 바꾸었으므로 `register` / `installSection` / `installSettingsSection`을
> 전혀 호출하지 않습니다.
> 드리프트 가드: `tools/check-api-drift.ps1`(기본적으로 `dsh-v0.1.5-rc.2`에 대해 필수 API 존재를 검증).
...
> ⚙️ **피크 정책은 설정 가능**: 시간대, 타임존, 윈도우의 요일 제한, 주말 규칙은 모두 `config/session-guard.json`이 결정하며 하드코딩이 아닙니다. 또한 **畅跑** — 단일 세션의 시간 한정 피크 면제(아래 "畅跑(free-run)" 참조).

## 기능

- **설정 가능한 피크 정책**: 피크/주말 정책이 `config/session-guard.json`에 정의됩니다(4단계 해석 순서, 자동 생성되는 설정 폼에서 명시한 값이 우선). 3가지 모드 `OFF_PEAK / PEAK / NORMAL`, 다중 윈도우·자정 횡단·요일 제한 지원. 잘못된 파일은 fail-open, `reloadConfig`로 핫 리로드.
...
```bash
# DSH 0.1.7-rc.x 호스트(본 라인, dist-tag dsh-0.1.7)
...
`compat/0.1.7`는 DSH `0.1.7-rc.x` 전용 라인이며 npm 버전은 **`3.1.1`**입니다. DSH `0.1.5-rc.x`
호스트는 **`compat/0.1.5`** 브랜치(npm dist-tag `dsh-0.1.5`, 버전 `3.0.0`), DSH `0.1.2-rc.x`
...
| `offPeakAutoResume` | on | **오피크 자동 재개**: 오피크에 본 플러그인이 정지한 세션을 자동 재개 |
...
- `timezone` (기본값 Asia/Shanghai) — IANA 이름으로 **모든** 시간 판정(요일 / 주말 / 윈도우 일치)을 구동. `peakTimezone`은 피크 윈도우 판정만 덮어씀(아래 "설정 가능한 피크 정책" 참조)；
- `peakWindows` / `peakPolicy` (기본값 09:00–12:00 / 14:00–18:00, 평일) — 윈도우와 주말 규칙의 권위 있는 출처는 설정 파일 `config/session-guard.json`입니다. 자동 생성 폼에서 변경한 값이 최우선이라는 점은 그대로입니다；
...
- **피크 타임존**: 설정으로 결정됩니다 — `peakTimezone`(생략 시 `timezone`을 따름)이 피크 윈도우 판정을 구동하며 기본값은 `Asia/Shanghai`(DeepSeek 공식 과금 기준과 일치). v0.2.0의 "항상 북경 시간"은 더 이상 하드코딩이 아닙니다(아래 "설정 가능한 피크 정책" 참조)；
...
- **게이트 조건** (모두 충족): `enabled` + `stepLevelPause` + `step > 1` + 피크(설정된 피크 타임존, 기본 북경 시간, 주말 아님) + 대상 provider가 공식(`providerGuard`, 끄면 전부) + 요청급 hold 아님 + **畅跑**로 면제되지 않음 + 이번 피크 구간에서 수동 스킵 아님；
...
- **이벤트 push**: `GET /session-guard/events?session=<id>`(SSE)가 step 게이트 상태 변화 시 `step` 이벤트를 즉시 전달합니다(클라이언트는 폴링을 기다리지 않고 수렴)；
- **영속화 안 함**: 보류는 프로세스 내 Promise, 재시작 시 소멸(유령 상태 방지).

### 설정 가능한 피크 정책 (`config/session-guard.json`)
...
- **중국 법정 공휴일은 의도적으로 인식하지 않습니다**: 공휴일 캘린더가 없으므로 평일에 해당하는 법정 공휴일은 평범한 평일로 취급됩니다 — 아래 "기본 피크 정의, 그리고 '중국 법정 공휴일은 인식하지 않습니다'" 참조.
...
| `peakPolicy.peakWindows[].name` | — | 윈도우 이름(`/peak` 응답과 `/status`의 `windowName`에 표시) |
...
- 상단 툴바의 텍스트 버튼 3개:
  1. `新建畅跑任务` — 인라인 폼(`开始` / `结束`, 각각 **듀얼 먼스 날짜 범위 달력** + 시간 셀렉트, **시간 단위**: 시작 시 H = `H:00`, 끝 시 H = `H:59`, 즉 **끝 시의 한 시간 전체가 포함**)을 열고 닫음. `确定` / `取消` 포함;
...
**새 작업 폼의 듀얼 먼스 날짜 범위 달력**:

- 날짜 선택기를 열면 **두 달이 나란히** 표시되고 `‹‹ ‹  2026年 9月   2026年 10月  › ››`(작년 / 지난달 / 다음달 / 내년)로 이동할 수 있어 달을 넘는 범위도 한눈에 고를 수 있습니다;
- 주 시작은 **월요일**(一 二 三 四 五 六 日)이고, 오늘은 테두리로 표시되며, 선택한 범위는 모든 날에 **배경 강조**, 양 끝점은 **채워진 원**, 앞뒤 달의 보조 칸은 흐리게 표시됩니다;
- **첫 번째 클릭이 시작, 두 번째 클릭이 끝**을 고릅니다. 끝이 시작보다 앞서면 **자동으로 교체**되며(역순 선택 가능), 범위가 채워지면 달력이 **자동으로 닫힙니다**. 트리거에는 `시작 → 끝`이 표시됩니다(양 끝이 미선택이면 플레이스홀더);
- 패널을 열 때 **현재 시각**으로 초기화합니다(시작 = 현재 시의 정각 `H:00`, 끝 = **다음 시**의 `H+1:59` — 즉 시계 두 시간. 날을 넘기면 다음 날로 이월) — "畅跑 작업 생성"의 의도는 거의 항상 "지금 시작"이기 때문입니다. 시간 셀렉트는 시간 정밀도를 위해 달력과 나란히 남고, 폼의 `现在` 버튼으로 언제든 현재 시각으로 **다시 시드**할 수 있습니다.
...
- 작업은 **절대 기시각/종시각**이고 반개구간 `[from, to)`, **시간 정밀도이며 끝 시가 통째로 포함**됩니다: 시작 시 H는 `H:00`, 끝 시 H는 `H:59`입니다(`"시작 12시 → 끝 14시"` = `12:00 → 14:59`로 12·13·14시 세 시간을 덮고, `"시작 12시 → 끝 12시"`는 정확히 그 한 시간만 덮습니다. `"끝 23시"` = `23:59`라 하루의 마지막 한 시간 23:00–24:00도 고를 수 있습니다). 호스트의 윈도우는 여전히 반개구간 `[from, to)`이며 선택기가 `:59`를 넘겨줄 뿐입니다. 폼의 값은 설정된 `timezone`으로 해석됩니다(패널이 그 타임존을 표기하며, `peakWindows`와 주말 규칙과 같은 타임존);
- `from`은 **곧**일 수도 **아주 먼 미래**일 수도 있습니다. 현재보다 이른 `from`은 현재 시각으로 클램프되므로 "즉시 시작"이 동작합니다;
- **일회성**: 각 작업은 `to`에서 자동 종료되고 더 이상 매칭되지 않습니다 — 이것은 정상 수명 주기이며 **설정 오류가 아니므로 오류로 보고되지 않습니다**(만료 임박 경고도 없습니다). 사용자는 언제든 새 작업을 추가할 수 있습니다;
...
- 畅跑가 효력을 가지는 동안 그 세션은 어디에도 보류되지 않습니다: 자동 턴급 / step 게이트 일시정지는 건너뛰고, 원래 `agent/request`에서 보류될 요청도 통과시킵니다(이를 위해 **세션 단위 `release`**를 추가해 이미 보류 중인 요청을 진행시킵니다) — 즉 면제는 피크 진입 시점, step 게이트, `agent/request` 세 곳에서 모두 적용됩니다;
...
- `pause`는 기존 세션 게이트를 재사용합니다: 일시정지 스냅샷을 저장하고 안전 경계를 기다리며, **`pausedReason`도 기록**합니다 — 피크 정책 정지는 `"peak_window"`, 명시적 `/pause`는 `"manual"`；
...
`enabled`, 또는 설정 폼), 그날 피크 윈도우에서 일시정지되는 것을 받아들이는 것입니다.
...
- 타임존 판정은 **IANA 타임존 이름**(예: `Asia/Shanghai`, `Asia/Tokyo`, `Asia/Seoul`)에 기반하며, `Intl.DateTimeFormat`으로 설정 타임존의 벽시계에 투영합니다 — **맨 `getUTCDay()`에 의존하지 않습니다**(북경 시간 UTC+8 경계에서 8시간 어긋나는 고전적 버그 회피: 북경 시간 토요일 00:30은 UTC로 아직 금요일)；
- `timezone`(기본값 `Asia/Shanghai`)이 **모든** 판정(요일·주말·피크 윈도우 일치)을 구동합니다(이전 버전에서는 피크 윈도우가 북경 시간 고정이었지만 더 이상 하드코딩이 아닙니다)；
- `peakTimezone`은 선택 사항이며 **피크 윈도우 판정만** 덮어씁니다 — 피크를 DeepSeek 과금 타임존에 고정하면서 주말은 로컬 `timezone`을 따르게 할 수 있습니다. 생략 시 `timezone`을 따릅니다；
- **타임존 문자열은 IANA 데이터베이스로 검증**됩니다: 설정 파일의 잘못되거나 철자가 틀린 값(예: `Asia/Shangai`)은 **거부되고 기본값이 유지되며**, 이유가 `configFile.errors`에 기록됩니다 — 벽시계 투영까지 가서 `RangeError`를 던지고 그것이 삼켜져 가드가 조용히 무효화되는 경로로 들어가지 않습니다；
- `Intl.DateTimeFormat`은 마지막 검증 계층이기도 합니다: 잘못된 타임존 이름(예: `Foo/Bar`)은 `RangeError`를 던지고 fail-open으로 기본 `Asia/Shanghai`로 폴백합니다；
- 피크 윈도우는 **좌폐우개** `[start, end)`이며 자정 횡단 윈도우(예: `22:00–06:00`, **시작일**에 귀속)를 지원합니다；
- 畅跑 작업의 기시각/종시각도 마찬가지로 `timezone`으로 해석됩니다(시간 정밀도·절대 시각. 시작 시 = `H:00`, 끝 시 = `H:59`로 끝 시 한 시간 전체가 포함되며 `"끝 23시"` = `23:59`)；
- `timezone` 설정은 모든 UI 언어(중/영/일/한)에서 동일하게 동작합니다 — IANA 타임존 이름은 locale에 의존하지 않습니다.
...
- 15초마다 `GET /session-guard/status` 폴링(`phase` / `providerGuard` / `held` / `deferred` / `stepHeld`. `/status`는 `mode` / `reason` / `windowName` / `minutesUntilPeak` / `peakTimezone` / `weekendDays` / `configFile`도 반환하며 `phase`는 호환을 위해 유지)；
...
- **畅跑와 무관**: 배지는 **전역** 피크 상태이고 畅跑는 **세션급** 면제이므로, 畅跑가 효력을 가져도 배지는 바뀌지 않습니다；
