...
- **畅跑小时语义（结束包含）**：开始小时 H = `H:00`，结束小时 H = `H:59`，因此结束小时整整一小时都被包含（「开始 12 时 → 结束 14 时」= `12:00 → 14:59`，覆盖 12、13、14 三个小时；「开始 12 时 → 结束 12 时」只覆盖那一小时；「结束 23 时」= `23:59`，一天最后一小时 23:00–24:00 也能选到）。宿主窗口仍是半开区间 `[from, to)`，选择器只是把 `:59` 交给它。
...
- **新模块**：`src/free-run.js`（畅跑模型 + 存储）、`src/client/free-run-button.tsx`、`src/client/free-run-button-text.ts`（文案与状态投影，纯函数可单测）；`TimePolicyResolver`（时间策略解析）。测试新增 `tests/free-run.test.mjs`（25）、`tests/free-run-isolation.test.mjs`（7）、`tests/free-run-button-text.test.mjs`（10）；`tests/config-file.test.mjs` 为 29、`tests/date-range.test.mjs` 为 20、`tests/index-apply.test.mjs` 为 23。全套 **390 通过**。
...
- **畅跑交互重设计（同一 PR 内改定）**：按钮文案固定为「畅跑」/「畅跑 ×N」，**点击永远打开管理面板**（旧版「畅跑中」点击会直接暂停畅跑，导致生效期间面板不可达——该死角已消除）；暂停从**会话级**改为**每个任务一级**，会话级启停开关被移除；会话级 `freeRunSuspend` / `freeRunResume` 动作（不带 `id`）被移除，改为按任务 `id` 的 `freeRunPause` / `freeRunResume` 与工具栏对应的 `freeRunPauseAll` / `freeRunResumeAll`。窗口语义（单会话、绝对起止时刻 `[from, to)`、`from` 钳到此刻、一次性到点结束不报错、落盘、上限 8）全部不变；小时精度此后细化为**结束包含**（开始小时 = `H:00`、结束小时 = `H:59`，见上）。
