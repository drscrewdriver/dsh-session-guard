...
- 上部ツールバーの 3 つのテキストボタン：
  1. `新建畅跑任务`——インラインフォーム（`开始` / `结束`、それぞれ**双月の日付範囲カレンダー** + 時間セレクト、**時間粒度**）を開閉。`确定` / `取消` 付き；
...
- パネルを開いた時点で**現在時刻**で初期化します（開始 = 現在の時間の正時、終了 = +2 時間。日をまたぐ場合は翌日に繰り越し）——「畅跑タスクを作る」の意図はほぼ常に「今すぐ開始」だからです。時間セレクトは時間精度のためにカレンダーと並んで残り、フォームの `现在` ボタンでいつでも現在時刻に**再シード**できます。
...
- 畅跑タスクの起止時刻も同様に `timezone` で解釈されます（時間精度・絶対時刻）；
