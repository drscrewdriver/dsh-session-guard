...
test('loadConfigFile + settingsBaseFor：文件值过不了 schema 时保住默认层（坏值被忽略）', async () => {
  const { settingsBaseFor } = await import('../src/settings.js')
  const p = j('C:\\proj', 'config', CONFIG_FILE_NAME)
...
test('0.1.7 声明式设置：Config 导出 schema，volatile 字段解析成 live ref（自动表单的前提）', async () => {
  const { SettingsSchema, DEFAULT_SETTINGS, canonicalSettings } = await import('../src/settings.js')
  const { Config } = await import('../src/index.js')
  assert.equal(Config, SettingsSchema, 'host 必须把 SettingsSchema 作为 Config 导出')

  // volatile 字段解析出来必须是 ref（带 .get()）—— dsh 的自动表单与实时读取依赖它
  const raw = SettingsSchema({})
  assert.equal(typeof raw.weekendDays.get, 'function', 'weekendDays 应是 volatile ref')
  assert.equal(typeof raw.peakWindows.get, 'function', 'peakWindows 应是 volatile ref')
  assert.deepEqual(raw.weekendDays.get(), ['sat', 'sun'])

  // 解引后的规范化形态 = 普通值，且覆盖 DEFAULT_SETTINGS 的全部键
  const { value, error } = canonicalSettings({})
  assert.equal(error, null)
  for (const key of Object.keys(DEFAULT_SETTINGS)) {
    assert.ok(key in value, `schema 应包含 DEFAULT_SETTINGS 的键 ${key}`)
  }
  // 新峰谷字段确实进了 schema 且带默认值
  assert.deepEqual(value.weekendDays, ['sat', 'sun'])
  assert.equal(value.peakTimezone, '')
  assert.deepEqual(
    value.peakWindows.map((w) => w.name),
    ['morning', 'afternoon'],
  )
})
