...
- 상단 툴바의 텍스트 버튼 3개:
  1. `新建畅跑任务` — 인라인 폼(`开始` / `结束`, 각각 네이티브 날짜 선택기 + 시간 셀렉트, **시간 단위**: 시작 시 H = `H:00`, 끝 시 H = `H:59`, 즉 **끝 시의 한 시간 전체가 포함**)을 열고 닫음. `确定` / `取消` 포함;
...
```bash
npm test   # node --test "tests/*.test.mjs"（390개 통과: 타임존/피크 정책/설정 파일/주말/상태 기계/세션 게이트/畅跑/브리지/재시도）
```
