...
test('apply()：config/session-guard.json 作为默认层，组合条目（volatile）覆盖它', async (t) => {
  withTmpState(t)
  withConfigFile(t, CFG_MORNING)
  const { ctx, routes } = fakeCtx()
  // 组合条目：模拟 dsh 传给 apply 的 volatile live ref（需 .get() 解引）。
  apply(ctx, {
    weekendDays: { get: () => ['sun'] }, // 覆盖配置文件里的 ['sat','sun']
    providerGuard: { get: () => false },
  })

  const rec = await callRoute(routes[0], fakeReq('GET', '/session-guard/settings'), fakeRes())
  const cfg = JSON.parse(rec.chunks.join('')).settings
  assert.deepEqual(cfg.weekendDays, ['sun'], '组合条目必须覆盖文件默认层')
  assert.equal(cfg.providerGuard, false)
  // 组合条目没写的键仍来自配置文件
  assert.deepEqual(cfg.peakWindows.map((w) => w.name), ['morning'])
  assert.equal(cfg.timezone, 'Asia/Shanghai')
  // 已无 ask 字段
  assert.equal(cfg.peakWarningMinutes, undefined)
  assert.equal(cfg.peakAskTimeoutSeconds, undefined)
  assert.equal(cfg.peakDefaultAction, undefined)
  // 文件与组合条目都没写的键用内置默认
  assert.equal(cfg.stepLevelPause, true)
})

test('apply()：不带组合条目（apply(ctx)）时不会被 undefined 抹掉默认层', async (t) => {
  withTmpState(t)
  withConfigFile(t, CFG_MORNING)
  const { ctx, routes } = fakeCtx()
  apply(ctx) // config 缺省 = {}
  const rec = await callRoute(routes[0], fakeReq('GET', '/session-guard/settings'), fakeRes())
  const cfg = JSON.parse(rec.chunks.join('')).settings
  assert.deepEqual(cfg.peakWindows.map((w) => w.name), ['morning'], '文件默认层必须保住')
  assert.equal(cfg.providerGuard, true, '内置默认必须保住（不能被 undefined 覆盖）')
  assert.equal(cfg.enabled, true)
})

test('组合条目里的 volatile ref 抛错 / 裸值：都不该炸，坏值按未设置处理', async (t) => {
  withTmpState(t)
  withConfigFile(t, CFG_MORNING)
  const { ctx, routes } = fakeCtx()
  assert.doesNotThrow(() =>
    apply(ctx, {
      weekendDays: {
        get() {
          throw new Error('volatile ref blew up')
        },
      },
      timezone: 'Asia/Tokyo', // 裸值（非 ref）也要接受
    }),
  )
  const rec = await callRoute(routes[0], fakeReq('GET', '/session-guard/settings'), fakeRes())
  const cfg = JSON.parse(rec.chunks.join('')).settings
  assert.equal(cfg.timezone, 'Asia/Tokyo', '裸值应被采信')
  assert.deepEqual(cfg.weekendDays, ['sat', 'sun'], '抛错的 ref → 视为未设置，回退文件/默认')
})

// ══════════════════════════════════════════════════════════════════════
// 配置文件热重载（v0.3.0）：必须真的改变**生效配置**，而不只是诊断信息
//
// 0.1.7 线的优先级：config/session-guard.json（默认层）← 组合条目 volatile（覆盖层）。
// ══════════════════════════════════════════════════════════════════════
...
test('reloadConfig：改配置文件后**生效配置**真的变了', async (t) => {
  withTmpState(t)
...
test('reloadConfig：组合条目里的值仍然优先于新的文件值', async (t) => {
  withTmpState(t)
  const cf = withConfigFile(t, CFG_MORNING)
  const { ctx, routes } = fakeCtx()
  const ref = { current: ['sun'] }
  apply(ctx, { weekendDays: { get: () => ref.current } })
  ...
  const rec = await callRoute(routes[0], fakeReq('GET', '/session-guard/settings'), fakeRes())
  const cfg = JSON.parse(rec.chunks.join('')).settings
  assert.deepEqual(cfg.weekendDays, ['sun'], '设置表单里设过的键必须继续优先')
  assert.deepEqual(cfg.peakWindows.map((w) => w.name), ['night'], '没设过的键应采用文件新值')
...
test('组合条目完全缺失时 readCfg 也不炸（fail-open）', async (t) => {
  withTmpState(t)
  withConfigFile(t, CFG_MORNING)
  const { ctx, routes } = fakeCtx()
  assert.doesNotThrow(() => apply(ctx, {}))
